...
export default function RentsPage() {
  ...
  async function handlePrint() {
    ...
    setPrinting(true);
    try {
      // Fetch full tenant details in parallel
...
<div class="tenant">
  ...
  <div class="tinfo">
    <div class="tname">${String(t?.name ?? p.tenantName ?? "Unknown")}</div>
    <div class="grid">
      ...
      <div class="field"><label>Email</label><span>${String(t?.email ?? "—")}</span></div>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, sans-serif; }
  ...
  .tname { font-size: 15px; font-weight: 800; margin-bottom: 8px; }
  ...
  @media print {
    ...
    .floor-heading { break-after: avoid; background: #1a1a2e !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    @page { margin: 14mm; }
  }
</style>
</head>
            ...
            {payments.length > 0 && (
              <div className={styles.printRow}>
                <button className={styles.printBtn} onClick={handlePrint} disabled={printing}>
                  {printing ? "Preparing PDF…" : "🖨️ Print / Export PDF"}
                </button>
              </div>
            )}
